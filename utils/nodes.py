...
def _strip_redefined_helpers(code: str) -> str:
    """Remove user-defined helper functions that shadow built-ins."""
    ...
    while i < len(lines):
        ...
        if match and match.group(2) in _FORBIDDEN_HELPERS:
...
Generate only the Python code, wrapped in ```python``` blocks."""
        ...
        code = _strip_redefined_helpers(code)
...
class FormatResultsNode(Node):
    ...
    def exec(self, prep_res):
        ...
        if result.get("success"):
            ...
            # Promote raw Mermaid printed to stdout into a renderable artifact.
            if output and not any(art.get("type") == "mermaid" for art in artifacts.values()):
                mermaid_start = None
                mermaid_end = None
                mermaid_prefixes = (
                    "graph",
                ...
                if mermaid_start is not None:
                    for idx in range(mermaid_start + 1, len(lines)):
                        if lines[idx].strip().startswith("```"):
                            mermaid_end = idx
                            break
                    mermaid_block = "\n".join(lines[mermaid_start:mermaid_end]).strip()
                    if mermaid_block:
                        mermaid_id = f"mermaid_{str(uuid.uuid4())[:8]}"
                        artifacts[mermaid_id] = {"type": "mermaid", "content": mermaid_block}
